...
const generateUserRecommendations = async (userId, preferences) => {
    try {
        ...
        const recommendUrl = `${ML_API_BASE_URL}/recommend/content-based?activity_name=${encodeURIComponent(initialActivity.name)}&top_n=3`;
...
const regenerateUserRecommendations = async (userId, preferences) => {
    try {
        const categoriesParam = preferences.map(pref => pref.toLowerCase()).join(',');
        ...
        if (!updatedRecommendation) {
            throw new Error('No existing recommendations found to update');
        }
        ...
        console.error('Error regenerating user recommendations:', error);
        return {
            success: false,
            error: error.message
        };
